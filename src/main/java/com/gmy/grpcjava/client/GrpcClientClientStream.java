...
public class GrpcClientClientStream {
    ...
    public GrpcClientClientStream(Channel channel) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ManagedChannel channel = ManagedChannelBuilder.forTarget(target)
                ...
                .build();
        try {
            GrpcClientClientStream client = new GrpcClientClientStream(channel);
            // 客户端流模式
            client.clientStream();
    ...
    private void clientStream(){
        // 编码与双端流一样
        StreamObserver<GreeterOuterClass.HelloRequest> requestStreamObserver = greeterStub.sayHelloClientStream(new StreamObserveResponse());
        for (int i = 0; i < 10; i++) {
